...
const AttendanceUpdate = ({ id, props: { history } }) => {
  ...
  React.useEffect(() => {
    ...
    dispatch(getItem(attendancesActions, `${parameters}/${id}`));
  }, [dispatch, parameters, id, branchesparams]);

  const { register, handleSubmit, errors } = useForm();

  const { branches, attendances, configs, countries, states, cities } = useSelector((state) => state);

  const { config: data } = configs;

  const { attendance: defaults, loading } = attendances;
  ...
  const onSubmit = (data) => {
    dispatch(updateItem(attendancesActions, `${parameters}/${id}`, data));
  };
          ...
          <div className="bgc-white bd bdrs-3 p-20 mB-20">
            ...
            >
              ...
              {loading ? (
                ...
                <div className="form-attendance d-flex flex-row-reverse">

...
const GroupCreate = ({ match, ...rest }) => {
  ...
  const { groups, configs, zones, branches } = useSelector((state) => state);
  ...
  if (!isEmpty(match.params) && match.path === `/reports/${parameters}/:id`) {
    ...
    return <GroupUpdate id={id} props={rest} />;
  }

  if (isEmpty(match.params) &&
    match.path === `/reports/${parameters}`
  ) {
    return <GroupView match={match} props={rest} />;
  }

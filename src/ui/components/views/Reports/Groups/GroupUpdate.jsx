...
const GroupUpdate = ({ id, props: { history } }) => {
  ...
  React.useEffect(() => {
    ...
    dispatch(getItem(groupsActions, `${parameters}/${id}`));
  }, [dispatch, parameters, id, branchesparams]);

  const { register, handleSubmit, errors } = useForm();

  const { branches, groups, configs, countries, states, cities } = useSelector((state) => state);

  const { config: data } = configs;

  const { group: defaults, loading } = groups;
  ...
  const onSubmit = (data) => {
    dispatch(updateItem(groupsActions, `${parameters}/${id}`, data));
  };

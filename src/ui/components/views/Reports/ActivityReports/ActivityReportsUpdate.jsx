...
const ActivityReportsUpdate = ({ id, props: { history } }) => {
  ...
  React.useEffect(() => {
    ...
    dispatch(getItem(activityreportsActions, `${parameters}/${id}`));
  }, [dispatch, parameters, id, branchesparams]);

  const { register, handleSubmit, errors } = useForm();

  const { branches, activityreports, configs, countries, states, cities } = useSelector((state) => state);

  const { config: data } = configs;

  const { activityreport: defaults, loading } = activityreports;
  ...
  const onSubmit = (data) => {
    dispatch(updateItem(activityreportsActions, `${parameters}/${id}`, data));
  };
...
ActivityReportsUpdate.propTypes = {
...
export default ActivityReportsUpdate;

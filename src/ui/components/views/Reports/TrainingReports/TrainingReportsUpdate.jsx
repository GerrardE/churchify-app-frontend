...
const TrainingReportsUpdate = ({ id, props: { history } }) => {
  ...
  React.useEffect(() => {
    ...
    dispatch(getItem(trainingreportsActions, `${parameters}/${id}`));
  }, [dispatch, parameters, id, branchesparams]);

  const { register, handleSubmit, errors } = useForm();

  const { branches, trainingreports, configs, countries, states, cities } = useSelector((state) => state);

  const { config: data } = configs;

  const { trainingreport: defaults, loading } = trainingreports;
  ...
  const onSubmit = (data) => {
    dispatch(updateItem(trainingreportsActions, `${parameters}/${id}`, data));
  };
...
TrainingReportsUpdate.propTypes = {
...
export default TrainingReportsUpdate;
